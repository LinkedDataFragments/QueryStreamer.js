...
echo "clients csparq taquery cqels" > .scalability.data
...
for i in 10 100 200; do
    ...
    file="cqels-output/run-$i.txt"
    q=$(tail -n +2 $file | awk '{print $9}')
    echo "csparql taquery" > .scalability-100.data
    paste -d ' ' <(echo "$c") <(echo "$t") <(echo "$q") >> .scalability-100.data
