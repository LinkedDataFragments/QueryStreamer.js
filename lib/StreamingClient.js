...
StreamingClient.prototype.run = function(callback) {
  var self = this;
  taCounter = 0;
  var variableAccumulator = this.query.variables.slice(0);
  this._executeTransformation(this.query.prefixes, variableAccumulator, this.query.where, function(modifiedQuery, hasTimeAnnotation) {
    self.query.where = modifiedQuery;

    if(hasTimeAnnotation) {
      self.query.variables = variableAccumulator;
    ...
    var context = {counter: 0, hasTimeAnnotation: hasTimeAnnotation, distinct: self.query.distinct};
