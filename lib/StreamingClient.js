...
StreamingClient.prototype.run = function(callback) {
  ...
  this.query = this._bNodesToVariables(this.query);
  ...
  this._splitStaticDynamicQuery(this.query.prefixes, this.query.variables, this.query.where, function(staticHolder, dynamicHolder) {
    ...
    self._extractSingularStatic(staticHolder.prefixes, staticHolder.variables, staticHolder.where, intersectedVariables, function(dynamicResult) {
      if(dynamicResult) dynamicHolder = appendAllQueryHolders(dynamicHolder, [dynamicResult], false);
    });
 ...
 */
StreamingClient.prototype._bNodesToVariables = function(query) {
 ...
 */
StreamingClient.prototype._bNodesToVariablesInner = function(originalVariables, originalAndNewVariables, queryPart) {
  var self = this;
 ...
 */
StreamingClient.prototype._extractSingularStatic = function(prefixes, variables, where, intersectedVariables, resultsCallback) {
  ...
  if(where instanceof Array) {
    ...
    for(var i = 0; i < where.length; i++) {
      this._extractSingularStatic(prefixes, variables, where[i], intersectedVariables, function(dynamicHolder) {
        called++;
        if(dynamicHolder.where && (!Array.isArray(dynamicHolder.where) || dynamicHolder.where.length > 0)) {console.log("EXTRACT");
        ...
        if(called == where.length) { // If at the end of the loop
          resultsCallback(flattenQueryHolders(resultDynamicQuery));
        }
      });
    }
    ...
    this._extractSingularStatic(prefixes, variables, where.triples, intersectedVariables, function(dynamicHolder) {
      ...
      appendAllQueryHolders(resultDynamicQuery, [dynamicHolder], false);
    ...
    this._extractSingularStatic(prefixes, variables, where.patterns, intersectedVariables, function(dynamicHolder) {
      ...
      if(dynamicHolder) resultDynamicQuery.where = [];
      appendAllQueryHolders(resultDynamicQuery, [dynamicHolder], false);
      resultDynamicQuery.where.patterns = flattenQueryHolders([dynamicHolder]);
      resultsCallback(resultDynamicQuery);
    });
  } else {
    ...
    if(shouldExtract) {
      resultDynamicQuery = initQueryHolder();
      appendQueryPart(resultDynamicQuery, where, prefixes, variables);
      resultsCallback(resultDynamicQuery);
    } else {
 ...
 * afterwards.
 */
StreamingClient.prototype._populateStaticdata = function(context, row, disjointVariables, staticHolder, cache, callback) {
