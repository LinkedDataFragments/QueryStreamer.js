...
function Streamer(ldf, target, originalVariables, durationCallback, type, windowSlide, singleQuery) {
  if (!(this instanceof Streamer))
    return new Streamer(ldf, target, originalVariables, type, windowSlide);
  ...
  function endQueryBarrier(lastResults) {
    ...
    if(barrierCount == 2 && !barrierCalled) {
      ...
      if(context.hasTimeAnnotation) {
        ...
        if (self._windowSlide) {
          var newTimeOut = self._windowSlide - duration;
          useLastResults = newTimeOut < timeOut;
          timeOut = newTimeOut;// + self._windowSlide;
        }*/
