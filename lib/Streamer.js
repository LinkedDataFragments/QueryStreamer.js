...
function Streamer(ldf, target, originalVariables, durationCallback, type) {
  if (!(this instanceof Streamer))
    return new Streamer(ldf, target, originalVariables, type);
...
Streamer.prototype._populateTimeAnnotationImplicitGraph = function(context, row, callback) {
  var resultRows = [];
  var results = new this._ldf.SparqlIterator(context.timeAnnotationQuery(row), { fragmentsClient: new this._ldf.FragmentsClient(this._target) });
