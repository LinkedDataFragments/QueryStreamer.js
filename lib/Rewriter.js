...
Rewriter.prototype._shouldTransform = function(prefixes, variables, triple, callback) {
  ...
  if(this._type == "implicitgraphs") {
    subQuery= this._singularQuery([triple], prefixes);
    composer = new Composer(subQuery);
    result = composer.compose();
    this._isQueryNotEmptyConditional(result, function (bindings, innerCallback) {

        composer = new Composer(self._materializedImplicitGraphQuery(bindings, triple, prefixes));
        result = composer.compose();
 ...
 * @private
 */
Rewriter.prototype._materializedImplicitGraphQuery = function(bindings, triple, prefixes) {
  var boundTriple = Object.clone(triple, true);
  util.bindVariables(boundTriple, bindings);
  return this._implicitGraphQuery(boundTriple, prefixes);
};
 ...
 * @private
 */
Rewriter.prototype._implicitGraphQuery = function(triple, prefixes) {
  return this._singularQuery(util.timeAnnotate([], util.tripleToIRI(this._target, triple)), prefixes);
};
 ...
 * @private
 */
Rewriter.prototype._singularQuery = function(triples, prefixes) {
  var query = {
    "type": "query",
    "prefixes": this._extendObject({
      "tmp": util.prefixes.tmp,
...
Rewriter.prototype._isQueryNotEmpty = function(subQuery, callback) {
  ...
  results.on('data', function (row) {
    shouldCall && !(shouldCall = false && results.end()) && callback(true, row);
